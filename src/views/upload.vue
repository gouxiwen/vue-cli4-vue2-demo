  ...
  <div class="upload">
    ...
    <div class="fl">
      <el-upload
        ...
        <el-button class="text">上传到测试环境</el-button>
      </el-upload>
      测试访问地址:<a class="a" :href="uploadFileObj.imageUrl" target="_blank">{{uploadFileObj.imageUrl}}</a>
    </div>
    <div class="fl">
      <el-upload
        ref="upload"
        ...
        :http-request="uploadRequest">
        <el-button class="text">上传到生产环境</el-button>
      </el-upload>
      生产访问地址:<a class="a" :href="uploadFileObj.imageUrlPrd" target="_blank">{{uploadFileObj.imageUrlPrd}}</a>
    </div>
    <div @click="send">发消息</div>
    <div @click="send2">发消息2</div>
    <div @click="$router.push('/test1')">去test1</div>
    <a href="http://m.news.cctv.com/2020/04/19/ARTIsmqnDDKud0Ae7Db7ersm200419.shtml" target="_blank" class="a3" mon="ct=1&amp;a=1&amp;c=top&amp;pn=0">重温习近平"4·19"重要讲话 打好互联网战"疫"</a>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async beforeUploadTest(file) {
      ...
      let type = file.name.substr(file.name.lastIndexOf('.'), file.name.length - 1)
    ...
    uploadRequest() {
      this.upload(this.uploadFileObj.file, '',
        next => {
